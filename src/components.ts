...
namespace components {
  ...
  export class CrossTable {
    ...
    static crSettingsSwitch(
      ...
      if (autoStateChange) {
        _dev_CrosstableMethods.handleLabelListeners(label);
      }
  ...
  export class ExtensionSettings {
    ...
    private static crStyleSwitchLabel({
      ...
      const elemId = `ccc-crosstable-option-${Utils.intId()}`;
  ...
  export class CustomCrosstable {
      ...
      modalBackdrop.addEventListener("click", (e) => {
        ...
        if (
          ...
          target?.classList?.contains(
            this.cssClasses.crosstableOptionsWrapper
          ) ||
    ...
    private static crCustomCrosstable() {
      ...
      for (let i = 0; i < standings.length; i++) {
        ...
        const row = components.CustomCrosstable.crRow();
        ...
        for (let j = 0; j < standings.length; j++) {
          ...
          const col = components.CustomCrosstable.crH2HCell();
          ...
          const colResultsWrapper =
            components.CustomCrosstable.crH2HResultsWrapper();

          const scoreWrapper = components.CustomCrosstable.crH2HScoreWrapper(
  ...
  export class CustomSchedule {
     ...
     */
    private static binarySearchCurrentGame(arr: chess_com.schedule_entry[]) {
      let start = 0;
