...
(function () {
    ...
    const clockWrappers = document.querySelectorAll(".enginedata-clock-wrapper");
...
class CountCapturedPieces {
    ...
    static count(fen) {
        ...
        keys.forEach((key) => {
            if (key === "capturePoints" || key === "piecesLeft")
                return;
        ...
        if (this.boardState.capturePoints > 0) {
            const blacksScore = this.blackPiecesWrapper.querySelector("._ccc-capture-score");
            if (blacksScore) {
            ...
            const whitesScore = this.whitePiecesWrapper.querySelector("._ccc-capture-score");
            if (whitesScore) {
...
CountCapturedPieces.pieceAmountDefault = {
    K: 1,
    B: 2,
    N: 2,
    R: 2,
...
CountCapturedPieces.pieceValues = {
    K: 0,
    P: -1,
    R: -5,
    N: -3,
...
CountCapturedPieces.whitePiecesWrapper = document.querySelectorAll(".ccc-captured-pieces-wrapper")[1];
CountCapturedPieces.blackPiecesWrapper = document.querySelector(".ccc-captured-pieces-wrapper");
CountCapturedPieces.boardState = {
    // white pieces
    P: 0,
    B: 0,
    N: 0,
