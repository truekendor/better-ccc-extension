...
class ELO {
    static calculateEloFromPercent(percent) {
    ...
    static calculateErrorMargin(wins, draws, losses) {
        ...
        const difference = this.calculateEloDifference(devMax) - this.calculateEloDifference(devMin);
        const errorMargin = formatter.format(difference / 2);
        return `${errorMargin}`;
    }
    static calculateEloDifference(percentage) {
        return (-400 * Math.log(1 / percentage - 1)) / Math.LN10;
    }
    static calculateInverseErrorFunction(x) {
...
ELO.confidence = _a.confidenceIntervals["sigma2"];
