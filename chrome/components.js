...
(function (components) {
    ...
    class CrossTable {
        static crEngineNames() {
            ...
            const enginesRows = standingsRow.querySelectorAll(".font-extra-faded-white");
            enginesRows.forEach((row, index) => {
                row.textContent = `${index + 1} ${enginesNames[index]}`;
            });
        }
        static crPTNMLStat(ptnml) {
        ...
        static crSettingsSwitch(text, field) {
        ...
        static crPairsPerRowForm() {
            ...
            rowAmountInput.value = `${enginesAmount === 2
            ...
            formElement.addEventListener("submit", (e) => {
                ...
                document.body.style.setProperty("--custom-column-amount", `${value ? value * 2 : ""}`);
                if (enginesAmount === 2) {
         ...
         */
        static crAdditionalStatButton(stats, index_1, index_2) {
            ...
            additionalStatsBtn.addEventListener("click", (e) => {
                ...
                const infoElement = this.crAdditionalStatContent(stats, index_1, index_2);
         ...
         * into container
         */
        static crAdditionalStatContent(stats, index_1, index_2) {
            ...
            const p1 = this.crStatRow("Longest lossless streak: ", `${stats.longestLossless} pairs`);
            const p2 = this.crStatRow("Longest win streak: ", `${stats.longestWinStreak} pairs`);
            const p3 = this.crStatRow("Longest winless streak: ", `${stats.longestWinless} pairs`);
            const p4 = this.crStatRow("Performance: ", `${formatter.format(stats.performancePercent)}%`);
            const p5 = this.crStatRow("Pairs Ratio: ", `${stats.pairsWL[0]} / ${stats.pairsWL[1]} = ${formatter.format(stats.pairsRatio)}`);
        ...
        static crPTNMLEntries(element, ptnml) {
         ...
         */
        static crEloAndErrMarginElement(elo, margin) {
            const wrapper = document.createElement("div");
         ...
         */
        static crStatWrapperElement() {
            const wrapper = document.createElement("div");
            wrapper.classList.add("ccc-stat-wrapper");
            return wrapper;
        }
    }
    components.CrossTable = CrossTable;
    ...
    class Webpage {
         ...
         */
        static crCapturedPiecesWrapper() {
            const div = document.createElement("div");
            div.classList.add("ccc-captured-pieces-wrapper");
            return div;
        }
        static crTwitchChatExpandBtn() {
            const tableWrapper = document.querySelector("#righttable-righttable");
            const content = tableWrapper.querySelector("#righttable-content");
    ...
    class ExtensionSettings {
        ...
        static crExtensionSettingsModal() {
            ...
            // * ======
            // * ======
            this.crLineSeparator("Cross table"), this.crExtensionSettingRow({
            ...
            // * ======
            // * ======
            this.crLineSeparator("Main"), this.crExtensionSettingRow({
                ...
                tooltip: "Highlights the moves that were played in the reverse game and shows the point at which the game deviated. Consumes more traffic ~25-200kb per game",
                experimental: true,
            }));
            modalWrapper.append(this.crCloseBtn());
            crossTableModal.append(modalBackdrop);
        }
        static crLineSeparator(categoryName) {
            const wrapper = document.createElement("div");
        ...
        static crExtensionSettingRow({ key, description, tooltip, experimental = false, }) {
        ...
        static crCrossTableStylesForm() {
            ...
            form.addEventListener("change", (e) => {
                ...
                const crossTable = document.querySelector(".modal-vue-modal-content");
                if (!crossTable) {
                    return;
                }
                crossTable.setAttribute("data-style", UserSettings.customSettings.crosstablePairStyle);
            });
            return form;
        }
        static crStyleSwitchLabel({ count, labelText, inputValue, }) {
    ...
    components.ExtensionSettings = ExtensionSettings;
