...
class OnMessageHandlers {
    static async tabUpdateHandler(message) {
        ...
        const gameNumber = (await new Promise((res) => {
            if (tempGameNumber) {
                ...
                ExtractPageData.getCurrentGameNumber().then((gameNumber) => res(gameNumber));
            }
        }));
        const reverseGameCache = ChessGamesCache.getGame(GamePairHelper.getReverseGameNumber(gameNumber));
        if (!reverseGameCache) {
    ...
    static pgnResponseHandler(message) {
        if (!UserSettings.customSettings.highlightReverseDeviation ||
            message.type !== "reverse_pgn_response") {
            return;
        }
        const { payload } = message;
        const { pgn: reversePGN, reverseGameNumber,
        // gameNumber,
         } = payload;
        const currentPGN = ExtractPageData.getPGNFromMoveTable();
        if (!reversePGN) {
            console.log("OnMessageHandlers::pgnResponse handler:\nreverse pgn is null");
            return;
        }
...
browserPrefix.runtime.onMessage.addListener(function (message
// sender,
// senderResponse
) {
    try {
...
class HighlightDeviation {
    ...
    static highlight() {
        ...
        // todo change this
        if (FindTranspositions.agreementLength < 16)
            return;
    ...
    // todo add description
    static findTranspositionsAndHighlight(debugMessage = "") {
        if (!UserSettings.customSettings.highlightReverseDeviation) {
            return;
        }
        if (ExtractPageData.isMobile) {
    ...
    static highlightTranspositions() {
        FindTranspositions.transpositions.forEach((transpositionObject, index) => {
            this.moveTableElementList[transpositionObject.currentPly].classList.add("ccc-move-agree");
            this.moveTableElementList[transpositionObject.currentPly].title =
                "transposition";
            const isLastTransposition = FindTranspositions.transpositions.length === index + 1;
            if (!isLastTransposition) {
                return;
            }
            const reverseFenIndex = chessReverse.fields.FenHistoryTrimmed.indexOf(transpositionObject.fen);
            const reverseMove = chessReverse.fields.pgn[reverseFenIndex + 1];
            const moveElement = this.moveTableElementList[transpositionObject.currentPly + 1];
            if (!moveElement)
                return;
            const moveNumber = (reverseFenIndex + (reverseFenIndex % 2)) / 2 + 1;
            moveElement.title = `Deviated at ${moveNumber}. ${reverseFenIndex % 2 === 1 ? " " : " .."}${reverseMove}`;
            moveElement.classList.add("ccc-data-move");
            moveElement.setAttribute("data-move", reverseMove);
        });
    }
}
HighlightDeviation.moveTableElementList = [];
(function awaitNewGameGlobal() {
